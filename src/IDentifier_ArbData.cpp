	...
	if(arbPH)
	{
		*arbPH = PF_NEW_HANDLE( sizeof(ArbitraryData) );
		...
		if(*arbPH)
		{
			...
			arb_data->index = 0;
			strcpy(arb_data->name, "Object ID");
...
{
	// copy contents
	out_arb_data->index = in_arb_data->index;
	strcpy(out_arb_data->name, in_arb_data->name);
}
	...
	PF_ArbCompareResult	*compareP)
{
	...
	if(a_arbH && b_arbH)
	{
		...
		if( a_data->index == b_data->index && !strcmp(a_data->name, b_data->name) )
